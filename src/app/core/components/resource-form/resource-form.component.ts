...
import { ResourceForm, ResourceOption } from '@app/shared/models';
...
export class ResourceFormComponent {
  ...
  @Input()
  get formValue(): ResourceForm | undefined {
    return this._formValue;
  }
  set formValue(value: ResourceForm | undefined) {
  ...
  private _formValue: ResourceForm | undefined;
  ...
  @Output() submitted = new EventEmitter<ResourceForm>();
  @Output() typeChange = new EventEmitter<
    'http://localhost:8080/aria-api/api/classification/' | 'http://localhost:8080/aria-api/api/concordance/'
  >();
  ...
  handleSubmit(): void {
    if (this.form.valid) {
      const formValue: ResourceForm = this.form.value;
